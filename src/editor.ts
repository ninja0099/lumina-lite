...
function hexWithAlpha(hex: string, a: number): string {
  ...
  // 8-char hex (#rrggbbaa): extract alpha from hex, combine with passed alpha
  if (h.length === 8) {
    a = a * (parseInt(h.slice(6), 16) / 255);
    h = h.slice(0, 6);
  }
  if (h.length !== 6 || !/^[0-9a-f]{6}$/i.test(h)) return `rgba(0,0,0,${a})`;
...
export function createEditor(canvas: HTMLCanvasElement, getState: () => DesignState) {
  ...
  async function exportAnimation(
    ...
    try {
      ...
      for (let i = 0; i < frames; i++) {
        paintFrame(i / frames, canvas);
        await source.add(i / fps, 1 / fps, { keyFrame: i % fps === 0 });
        if (onProgress) onProgress(i + 1, frames, "Encoding (VBR)");
      }

      source.close();
      await output.finalize();
      downloadBlob(new Blob([output.target.buffer!], { type: output.format.mimeType }), outFileName);
    } catch (err) {
  ...
  async function exportAnimationWebM(
    ...
    try {
      ...
      const source = new CanvasSource(canvas, {
        codec: "vp9",
        fullCodecString: VP9_CODEC_10BIT,
        bitrate: bitrateMbps * 1_000_000,
        bitrateMode: "variable",
      });
      output.addVideoTrack(source, { frameRate: fps });
      ...
      for (let i = 0; i < frames; i++) {
        paintFrame(i / frames, canvas);
        await source.add(i / fps, 1 / fps, { keyFrame: i % fps === 0 });
        if (onProgress) onProgress(i + 1, frames, "Encoding (VP9 10-bit VBR)");
      }
